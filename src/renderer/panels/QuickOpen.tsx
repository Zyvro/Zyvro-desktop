...
import { rank, splitLine } from "../../shared/fuzzy"
...
function QuickOpenDialog() {
  ...
  }, [query, recents, liste.data])

  const index = Math.min(choisi, Math.max(0, resultats.length - 1))
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === "ArrowDown") {
                ...
                setChoisi((i) => Math.min(i + 1, resultats.length - 1))
          ...
          <div className="zy-scroll min-h-0 flex-1 overflow-y-auto py-1">
            ...
            {!liste.isLoading && resultats.length === 0 && (
